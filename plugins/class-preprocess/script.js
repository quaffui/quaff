 ...
 * @returns
 */
export function prepareScript(instance, source) {
  ...
  for (const node of instance.content.body) {
    ...
    if (object.name !== "Quaff" || property.name !== "classes") continue;
    ...
    if (expression.arguments.length !== 2)
      throw new Error("The Quaff.classes function takes exactly 2 arguments");

    const [component, options] = expression.arguments;

    if (component.type !== "Literal")
      throw new Error(
        "The Quaff.classes 1st argument should be a Literal containing the name of the component"
      );

    if (options.type !== "ObjectExpression")
      throw new Error("The Quaff.classes 2nd argument should be an object");

    if (options.properties.some((prop) => prop.type === "SpreadElement"))
      throw new Error("The Quaff.classes 2nd argument doesn't take SpreadElements as a property");
