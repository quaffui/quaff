...
- When adding a component with CSS, add its CSS wrapper and register its directory, stylesheet, public component names, and CSS dependencies together in `registry` in `src/lib/internal/componentRegistry.ts`. Omit `css` for groups without their own stylesheet; omit `components` for shared stylesheets. The CSS build entries and component/direct-import dependency maps are derived automatically. Preserve registry order because it controls the CSS cascade, and keep component exports in the public barrel static.
